...
@WebMvcTest(DesarrolladorControllerV2.class)
@AutoConfigureMockMvc(addFilters = false)
class DesarrolladorControllerV2Test {
    ...
    private EntityModel<DesarrolladorResponseDTO> entityModel() {
        return EntityModel.of(dto(),
                Link.of("http://localhost/api/v2/desarrolladores/1").withSelfRel());
    }
    ...
    void getAll_ConDatos_Retorna200() throws Exception {
        ...
        mockMvc.perform(get("/api/v2/desarrolladores")).andExpect(status().isOk());
    }

    @Test
    void getAll_SinDatos_Retorna204() throws Exception {
        when(desarrolladorService.findAll()).thenReturn(List.of());
        mockMvc.perform(get("/api/v2/desarrolladores")).andExpect(status().isNoContent());
    }
    ...
    void getById_Existe_Retorna200() throws Exception {
        ...
        when(desarrolladorAssembler.toModel(any())).thenReturn(entityModel());
        mockMvc.perform(get("/api/v2/desarrolladores/1")).andExpect(status().isOk());
    }

    @Test
    void getById_NoExiste_Retorna404() throws Exception {
        when(desarrolladorService.findById(99L)).thenThrow(new RecursoNoEncontradoException("No encontrado"));
        mockMvc.perform(get("/api/v2/desarrolladores/99")).andExpect(status().isNotFound());
    }
    ...
    void create_DatosValidos_Retorna201() throws Exception {
        ...
        when(desarrolladorAssembler.toModel(any())).thenReturn(entityModel());
        mockMvc.perform(post("/api/v2/desarrolladores")
    ...
    void update_Existe_Retorna200() throws Exception {
        ...
        when(desarrolladorAssembler.toModel(any())).thenReturn(entityModel());
        mockMvc.perform(put("/api/v2/desarrolladores/1")
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void delete_Existe_Retorna204() throws Exception {
        ...
        mockMvc.perform(delete("/api/v2/desarrolladores/1")).andExpect(status().isNoContent());
    }

    @Test
    void delete_NoExiste_Retorna404() throws Exception {
        doThrow(new RecursoNoEncontradoException("No encontrado")).when(desarrolladorService).deleteById(99L);
        mockMvc.perform(delete("/api/v2/desarrolladores/99")).andExpect(status().isNotFound());
    }
}