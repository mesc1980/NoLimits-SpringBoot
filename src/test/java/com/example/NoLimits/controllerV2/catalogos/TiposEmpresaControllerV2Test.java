...
class TiposEmpresaControllerV2Test {
    ...
    private EntityModel<TiposEmpresaResponseDTO> entityModel() {
        return EntityModel.of(dto(),
                Link.of("http://localhost/api/v2/empresas/10/tipos-empresa/1").withSelfRel());
    }

    @Test
    void listar_ConDatos_Retorna200() throws Exception {
        when(tiposEmpresaService.findAllByEmpresa(10L)).thenReturn(List.of(dto()));
        when(tiposEmpresaAssembler.toModel(any())).thenReturn(entityModel());
        mockMvc.perform(get("/api/v2/empresas/10/tipos-empresa"))
                .andExpect(status().isOk());
    }

    @Test
    void listar_SinDatos_Retorna204() throws Exception {
        when(tiposEmpresaService.findAllByEmpresa(10L)).thenReturn(List.of());
        mockMvc.perform(get("/api/v2/empresas/10/tipos-empresa"))
    ...
    void link_DatosValidos_Retorna201() throws Exception {
        when(tiposEmpresaService.link(10L, 1L)).thenReturn(dto());
        when(tiposEmpresaAssembler.toModel(any())).thenReturn(entityModel());
        mockMvc.perform(post("/api/v2/empresas/10/tipos-empresa/1"))
                .andExpect(status().isCreated());
    }

    @Test
    void unlink_Existe_Retorna204() throws Exception {
        doNothing().when(tiposEmpresaService).unlink(10L, 1L);
        mockMvc.perform(delete("/api/v2/empresas/10/tipos-empresa/1"))
                .andExpect(status().isNoContent());
    }
}